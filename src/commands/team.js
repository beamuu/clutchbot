    ...
    try {
        ...
        if (teamNumber > participants.length) {
            const limitEmbed = new MessageEmbed()
                ...
                .setDescription(`[🙁] You are going to split in to ${teamNumber} team(s) which is more than the actual participants.`)
        ...
        if (teamNumber > 128) {
            const limitEmbed = new MessageEmbed()
                ...
                .setDescription(`[🙁] You can not spilt your participants in to more than 128 teams because of memory limitation on production deployment.`)
            return msg.channel.send({embeds: [limitEmbed]})
        }
        ...
        const errorEmbed = new MessageEmbed()
                ...
                .setFooter('[Tips] Team number must be integer and participants name can be a string without space(" ") or a Discord tag(@user), split each argument with space(" ").')
